...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			else if (key.compare("POSEIDON_M") == 0){
				conf >> poseidon_para_m;
				std::cout << "POSEIDON_M\t\t\t\t" << poseidon_para_m << '\n';
			}
			else if (key.compare("POSEIDON_MIN_RATE") == 0){
				conf >> poseidon_min_rate;
				std::cout << "POSEIDON_MIN_RATE\t\t\t\t" << poseidon_min_rate << '\n';
			}
			else if (key.compare("POSEIDON_MAX_RATE") == 0){
	...
	else if (cc_mode == 11) // poseidon, use int
		IntHeader::mode = IntHeader::NORMAL;
